...
local function is_Async_T(handle)
    -- if type(handle) == "table" and vim.is_callable(handle.cancel) and vim.is_callable(handle.is_cancelled) then
    --     return true
    -- end
    return handle and getmetatable(handle).__index == Async_T
end
...
local function add(name, argc)
    local success, ret = pcall(M.wrap, vim.uv[name], argc, true)
    if not success then
